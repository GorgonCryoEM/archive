...
void FlexibleFittingEngine::computeCorrespondencePairWiseWeight(const int& selectedAlignmentIndex)
{
    ...
    {            
        ...
        sourceMinX = std::min<float>(std::min<float>(sourceMinX, it->GetEndPoint(0).X()), it->GetEndPoint(1).X());
        sourceMinY = std::min<float>(std::min<float>(sourceMinY, it->GetEndPoint(0).Y()), it->GetEndPoint(1).Y());
        sourceMinZ = std::min<float>(std::min<float>(sourceMinZ, it->GetEndPoint(0).Z()), it->GetEndPoint(1).Z());
        sourceMaxX = std::max<float>(std::max<float>(sourceMaxX, it->GetEndPoint(0).X()), it->GetEndPoint(1).X());
        sourceMaxY = std::max<float>(std::max<float>(sourceMaxY, it->GetEndPoint(0).Y()), it->GetEndPoint(1).Y());
        sourceMaxZ = std::max<float>(std::max<float>(sourceMaxZ, it->GetEndPoint(0).Z()), it->GetEndPoint(1).Z());
        //std::cout << "s\n";
    ...
    {
        targetMinX = min<float>(min<float>(targetMinX, it->GetEndPoint(0).X()), it->GetEndPoint(1).X());
        targetMinY = min<float>(min<float>(targetMinY, it->GetEndPoint(0).Y()), it->GetEndPoint(1).Y());
        targetMinZ = min<float>(min<float>(targetMinZ, it->GetEndPoint(0).Z()), it->GetEndPoint(1).Z());
        targetMaxX = max<float>(max<float>(targetMaxX, it->GetEndPoint(0).X()), it->GetEndPoint(1).X());
        targetMaxY = max<float>(max<float>(targetMaxY, it->GetEndPoint(0).Y()), it->GetEndPoint(1).Y());
        targetMaxZ = max<float>(max<float>(targetMaxZ, it->GetEndPoint(0).Z()), it->GetEndPoint(1).Z());
        //std::cout << "t\n";
        //std::cout << std::setprecision(30) << it->GetEndPoint(0).X() << ", " << it->GetEndPoint(0).Y() << ", " << it->GetEndPoint(0).Z() << "\n";
        //std::cout << std::setprecision(30) << it->GetEndPoint(1).X() << ", " << it->GetEndPoint(1).Y() << ", " << it->GetEndPoint(1).Z() << "\n";
    }
    ...
    double sigma = m_bbRatio * min<float>(sourceBBDiagonal, targetBBDiagonal) / 3.0;
